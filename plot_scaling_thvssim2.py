...
ax=plt.axes()
ax.loglog(gamma,list_means/np.sqrt(list_meanvars),"o",color="blue")
ax.loglog(gamma2,list_means2/np.sqrt(list_meanvars2),"o",color="purple")
plt.show()

##### PLOTTING N RHO/sigma2 #####
if plotrho:
  ax = plt.axes()
  ax.loglog(gamma,N*list_rho*alphabar**2/list_meanvars,"v",label="N="+str(N),color="blue")
  ax.loglog(gamma2,N2*list_rho2*alphabar**2/list_meanvars2,"^",label="N="+str(N2),color="purple")

  rhosig2 = gamma
  correctedrhosig2 = rhosig2 + (theta2N[0]+theta2N[1])/2 * alphabar**2/sigma2_th
                                #recall |theta| = (theta2N[0]+theta2N[1])/(2*N)


  ax.loglog(gamma,gamma,label="Prediction",color=listcolors(0.6))
  ax.loglog(gamma,correctedrhosig2,label="Prediction (corrected)",color="red")
  ax.set_xlabel(r"$\gamma$")
  ax.set_ylabel(r"$N_e \rho \bar\alpha^2/\sigma^2$")
  ax.legend()
  plt.show()


##### PLOTTING AUTOCORRELATION #####
list_rhoth = correctedrhosig2 * list_meanvars2/(N2*alphabar**2)

if plotautocor:
  ax=plt.axes()

  for k in np.arange(0,nbpoints2):
    ax.plot(list_autocor[k],color=listcolors(k/nbpoints2),marker="v",alpha=0.05)
    ax.plot(np.exp(-list_rhoth[k]*np.arange(limit_autocor)),color="black")
