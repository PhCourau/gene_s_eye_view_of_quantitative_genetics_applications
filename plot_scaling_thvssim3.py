...
list_autocor = np.zeros((nbpoints,limit_autocor)) #Autocorrelations
...
for k in range(nbpoints):
  ...
  if plotautocor:
    ...
    for j in range(1,limit_autocor):
      list_autocor[k,j] =np.cov(tmp[3][-postburnin:-j],tmp[3][-postburnin+j:])[0,1]/list_varmeans[k]
...
if plotautocor:
  ...
  for k in np.arange(0,nbpoints):
    ax.plot(-np.log(list_autocor[k]),"o",color=listcolors(k/nbpoints))
    ax.plot(list_rho[k]*np.arange(limit_autocor),color=listcolors(k/nbpoints))

  #Color scale
  gammamin,gammamax=(np.min(gamma),np.max(gamma))
  # Normalizer
